...
function TimerPublic(){
  ...
  const d=sp.get('d')
  const [timer,setTimer]=useState(d?decodeB64Url(d):null)
  ...
  return(
    <div>
      <Backdrop/>
      <NeonHeader right={<button onClick={()=>setOpen(true)} className='text-xs bg-white/10 px-2 py-1 rounded'>Погода</button>}/>
      {!timer && <div className='px-4 pt-8 opacity-70'>Таймер не найден.</div>}
      {timer && (
        <div className='px-4 pt-6'>
          <div className='glass p-6 rounded-2xl' style={{backgroundImage:bg}}>
            <div className='text-xs opacity-70'>
              {timer.kind==='abs' ? new Date(timer.endsAt).toLocaleString() : 'длительный таймер'}
            </div>
